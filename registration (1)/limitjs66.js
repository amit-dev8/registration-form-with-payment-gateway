			...
			*/
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (status === 'exists') {
						$('#err_limit').text('Session Is Sold Out');
...
/*			if (cmorning1 === '') {
				$('#err_limit2').text('Please select a sessions');
				return;
			}
			*/
			$.ajax({
				...
				success: function (response) {
					...
					if (status === 'exists') {
						...
						//$('#err_limit2').text('No sessions provided');
					} else {
						$('#err_limit2').text(response);
					}
				},
				error: function () {
					$('#err_limit2').text('Request error');
				}
			});
		}
...
/*			if (cmorning1 === '') {
			...
			$.ajax({
				...
				success: function (response) {
					const status = (response || '').trim();
					if (status === 'exists') {
						$('#err_limit2').text('Session Is Sold Out');
						// Disable only the currently selected option
...
/*			if (cmorning1 === '') {
				$('#err_limit3').text('Please select a sessions');
				return;
			}
			*/
			$.ajax({
				...
				success: function (response) {
					...
					if (status === 'exists') {
						...
						//$('#err_limit3').text('No sessions provided');
					} else {
						$('#err_limit3').text(response);
					}
				},
				error: function () {
					$('#err_limit3').text('Request error');
				}
			});
		}
